...
const check = (s: Snapshot, id: string, r = req()) => analyze(r, s, now).checks.find(c => c.id === id)!;

describe('integer quote policy', () => {
  it('uses 18/6 decimal base units and floors the exact minimum', () => {
    const r = req(), s = fixture(r), out = analyze(r, s, now);
  ...
  it('handles the reverse direction without floating point amounts', () => {
    const r = req({ tokenIn: 'USDC', amount: '250' }), s = fixture(r);
...
describe('freshness and incomplete data fail closed', () => {
  it.each(['stale', 'risky'])('blocks the explicit %s sample', scenario => {
    const r = req({ scenario }); expect(analyze(r, fixture(r), now).decision).toBe('blocked');
  });
  it.each([-121, 31])('blocks block timestamp offset %is', offset => {
    const s = fixture(); s.block.timestamp = now + offset;
    expect(check(s, 'block').status).toBe('block');
  });
  it.each([
    { updatedAt: now - 3601 }, { updatedAt: now + 31 }, { updatedAt: 0 },
    { answerRaw: '0' }, { answerRaw: '-1' }, { answeredInRound: '99' },
    { roundId: '0' }, { decimals: 19 }, { decimals: -1 }, { decimals: 8.5 },
  ])('rejects an invalid oracle round %j', fields => {
    const s = fixture(); Object.assign(s.feeds.ETH, fields);
    expect(validFeed(s.feeds.ETH, now)).toBe(false);
    const out = analyze(req(), s, now);
    expect(out.decision).toBe('blocked'); expect(out.quote.referenceOut).toBeNull();
  });
  it('does not interpret absent wallet data as safe', () => {
    const s = fixture(); s.wallet = null;
  ...
  it('keeps failed reads unknown and low balance blocked', () => {
    const s = fixture(); s.wallet!.balanceRaw = null; s.wallet!.allowances[0].amountRaw = null;
    expect(check(s, 'balance').status).toBe('unknown');
    expect(check(s, 'allowance').status).toBe('unknown');
    s.wallet!.balanceRaw = '0'; expect(check(s, 'balance').status).toBe('block');
  });
  it('distinguishes high allowance exposure from execution readiness', () => {
    const s = fixture(); s.wallet!.allowances[0].amountRaw = '1000000000000000001';
    expect(check(s, 'allowance').status).toBe('warn');
    s.wallet!.allowances[0].amountRaw = '0';
    expect(check(s, 'allowance').detail).toContain('Zero allowance is not execution-ready');
  });
});

describe('strict request boundary', () => {
  it.each(['0', '-1', '1e3', 'NaN', 'Infinity', '.1', '1.', '1,000', '1001', '0.0000000000000000001'])('rejects WETH amount %s', amount => {
  ...
  it.each([0, -1, 1001, 1.5, '50', null])('rejects invalid slippage %s', slippageBps => {
    expect(() => req({ slippageBps })).toThrow();
  });
  it.each([{ mode: 'other' }, { tokenIn: 'ETH' }, { scenario: 'other' }, { mode: 'live', scenario: 'risky' }, { owner: '0x123' }, { owner: '0x0000000000000000000000000000000000000000' }])('rejects unsupported parameters %j', fields => {
  ...
  it('drops a real owner from sample data and does not accept RPC URL injection', () => {
    ...
    const live = req({ mode: 'live', owner, rpcUrl: 'https://attacker.example' });
    expect(live.owner).toBe(owner); expect(live).not.toHaveProperty('rpcUrl');
  });
  it('serializes reports with exact amounts as strings', () => {
    const r: AnalyzeRequest = req(), out = analyze(r, fixture(r), now);
    expect(JSON.parse(JSON.stringify(out)).quote.amountInRaw).toBe(out.quote.amountInRaw);
  });
});
