...
## Optional receipt verification and reference-agent comparison — 2026-09-10

- UI states now distinguish a completed check from an unfinished swap. Expected WAIT/REJECT is not styled/described as a failed application; unavailable output is not zero cost. The manual panels and historical evidence are preserved.
- `npm test`: **306/306 tests across 15 files**. New coverage includes 30 verifier tests (transaction/receipt binding, logs, price conversion, confirmations, reorg detection during a check, malformed/forged fields and journal restart), source/artifact integrity and server-rendered UI content. No new local browser interaction or visual QA is claimed.
- TypeScript/Vite production build passed. Existing Zod/Rollup annotation warnings are non-fatal. `npm run check:mcp` passed its 4-tool, 14-call legacy transport/restart check. `node --import tsx scripts/check-protection.ts` passed 25/25 synthetic cases; the initial npm/tsx CLI invocation was blocked by sandbox IPC permissions, not a product test failure.
- `node --import tsx scripts/check-rpc-mcp.ts`: the actual 5-tool stdio service passed a 6-call negative smoke against the local node, retaining pending state after 3 expected errors (missing transaction, caller cost injection and forged verification). RPC details were not exposed. No transaction was sent by this negative check.
- The predeclared `experiments/AGENT_RPC_PROTOCOL.md` was written before execution. Fresh Anvil 1.7.1 at Ethereum block **25938600** was used with fake development inventory, exact WETH approval, 0.04 WETH input, fee-500 Uniswap pool, fixed 1 gwei and identical reset snapshots. An initial publicnode provider could not serve historical state; the completed run used eth.drpc.org for upstream reads. No real wallet, public testnet or mainnet transaction was used.
- `rpc-agent-2026-09-10` records **12 task runs / 3 policies / 4 hand-designed cases**, actual local EVM receipts and one MCP restart between a failed swap and retry. Separate verifier reads and MCP-derived values matched. Recorded gas is gas used × effective price, converted using both receipt-block asset feeds. All original minimums and costs remain in the published transcript.
- Per-attempt baseline: **4/4 completed, 2/4 compliant, 2/4 overruns**, total task gas **2.219848 USDC-eq**. Independent cumulative baseline and MCP each: **3/4 completed, 2/4 compliant, 1/4 overruns**, total gas **1.880340 USDC-eq**. Totals include unfinished tasks. The tight-retry rejection trades one completion for one fewer overrun; underestimated gas causes an overrun in every policy. Equal cumulative/MCP results demonstrate integration consistency, not optimization superiority. Fault-injected cases are not market observations or independent statistical samples.
- The original synthetic batch bytes remain unchanged. Its source fingerprints are historical. The new RPC batch's artifact digest and exact current exercised source hashes are checked in `tests/rpc-agent-evidence.test.ts`; its source base commit identifies the checkout before uncommitted implementation, not a claim that that commit alone contains the new code.
